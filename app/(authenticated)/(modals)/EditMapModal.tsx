...
// import { RouteProp, useRoute } from "@react-navigation/native"; // To access route params (map)
// import { useMapsContext } from "@/app/contexts/MapsContext";
// import { Stack, useRouter } from "expo-router"; // To navigate back to ViewMapModal
...
//   mapId: string; // Map data will be passed as a stringified object
// };
...
//   const router = useRouter(); // Add router to navigate between screens

//   const map = maps.find((m) => m.id === route.params.mapId);

//   // Check if route.params exists and load the map
...
//   //   const [map, setMap] = useState<Map>(JSON.parse(route.params.map)); // Load map data, including existing pins
...
//       position: { x, y },
//     };
//     // const updatedMap = { ...map, pins: [...(map.pins || []), newPin] };
//     // setMap(updatedMap); // Update local map state with new pin
...
//     addPin(x, y); // Add pin when user taps on the map
//   };

//   const saveMap = () => {
//     // Save updated map (including pins) to the context
//     // updateMapPins(map.id, map.pins || []);

//     // // Navigate back to ViewMapModal with the updated map
//     // router.replace({
//     //   pathname: "/(authenticated)/(modals)/ViewMapModal",
//     //   params: { map: JSON.stringify(map) }, // Używamy zaktualizowanej mapy
//     // });
//     router.back();
//   };
